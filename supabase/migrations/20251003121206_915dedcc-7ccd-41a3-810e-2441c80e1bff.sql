-- Phase 1: Enable RLS on password_check_events table
ALTER TABLE public.password_check_events ENABLE ROW LEVEL SECURITY;

-- Policy: Users can only view their own password check events
CREATE POLICY "Users can view their own password check events"
ON public.password_check_events
FOR SELECT
USING (auth.uid() = user_id);

-- Policy: Users can only insert their own password check events
...
-- Policy: Only service role can manage roles (admin assignment)
